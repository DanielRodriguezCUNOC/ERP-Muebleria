...
public interface SpringDataUsuarioRepository extends JpaRepository<UsuarioJpaEntity, Long> {
    ...
    //* Esto sirve para verificar que almenos exista un administrador en la base de datos, para que no se pueda eliminar el unico administrador
    @Query(
            "SELECT COUNT(u) FROM UsuarioJpaEntity u WHERE u.rol.rol = 'ADMINISTRADOR' AND u.activo = true"
    )
